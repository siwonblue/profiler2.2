  ...
  return (
    <Col span={12}>
      <Row name="profile-card" style={a}>
        <Col name="profile-image" span={12} style={b}>
          <Link href="/profile/[id]" as={`/profile/${liking?.id}`}>
            <a>
              <img style={ImageStyle2} src={`${liking?.Images[0]?.src || `defaultProfile.jpeg`}`} alt="profile_image" />{' '}
            </a>
          </Link>
        </Col>
        <Col span={12} style={c}>
          <Row name="profile-meta">
            <Col span={24} style={d}>
              <Link href="/profile/[id]" as={`/profile/${liking?.id}`}>
                <a>{liking?.name.length > 5 ? <h4>{liking?.name.slice(0, 5)} ・・・</h4> : <h4>{liking?.name}</h4>}</a>
              </Link>
            </Col>
            <Col style={d} span={24}>
              <button onClick={onUnlike} style={e}>
                관심취소
              </button>
            </Col>
          </Row>
